...
*Setup instructions for a gitian build of HanaCryptoBank Core using a VM or physical system.*
...
to the HanaCryptoBank GitHub release page.
...
- [Building HanaCryptoBank Core](#building-hanacryptobank-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/hanacryptobank-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/hanacryptobank-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/hanacryptobank-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/hanacryptobank-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/hanacryptobank-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/hanacryptobank-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/hanacryptobank-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@hanacryptobank.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/hanacryptobank-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/hanacryptobank-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/hanacryptobank-osx-*-build.assert
```
