HanaCryptoBank Core version 2.3.0 is now available from:
...
HanaCryptoBank Core is extensively tested on multiple operating systems using the Linux kernel, macOS 10.8+, and Windows Vista and later.
...
[
  {
    ...
    winner: {
        address: addr,    (string) HanaCryptoBank MN Address,
...
[
  {
    ...
    "addr": addr,      (string) Masternode HanaCryptoBank address
...
HanaCryptoBank Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for HanaCryptoBank (Mrs-X)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with HanaCryptoBank (Fuzzbawls)
