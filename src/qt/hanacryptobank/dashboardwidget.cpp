...
#include "qt/hanacryptobank/forms/ui_dashboardwidget.h"
#include "qt/hanacryptobank/sendconfirmdialog.h"
#include "qt/hanacryptobank/txrow.h"
...
DashboardWidget::DashboardWidget(HanaCryptoBankGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of HCB staked."));
    ...
    ui->labelAmountPiv->setText("0 HCB");
    ...
    ui->btnHowTo->setText(tr("How to get HCB"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("HCB")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZHCB;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair HCB, zHCB
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("HCB");
    set1 = new QBarSet("zHCB");
