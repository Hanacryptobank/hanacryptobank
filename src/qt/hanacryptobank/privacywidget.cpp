...
#include "qt/hanacryptobank/privacywidget.h"
#include "qt/hanacryptobank/forms/ui_privacywidget.h"
#include "qt/hanacryptobank/qtutils.h"
#include "guiutil.h"
#include "qt/hanacryptobank/denomgenerationdialog.h"
#include "qt/hanacryptobank/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(HanaCryptoBankGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zHCB anonymizes your HCB by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zHCB or convert back to HCB"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 HCB ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zHCB");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zHCB");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zHCB");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zHCB");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zHCB");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zHCB");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zHCB");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zHCB");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zHCB Movements"));
    ...
    // Only Convert to HCB enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzHCB->setTitleClassAndText("btn-title-grey", "Total 0 zHCB");
    ui->btnTotalzHCB->setSubTitleClassAndText("text-subtitle", "Show denominations of zHCB owned.");
    ui->btnTotalzHCB->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select HCB outputs to mint into zHCB.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zHCB");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzHCB, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zHCB");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of HCB to mint into zHCB"));
    }else{
        btnText = tr("Convert back to HCB");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zHCB to convert back into HCB"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzHCB->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzHCB->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zHCB is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zHCB, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zHCB").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zHCB minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zHCB converted back to HCB"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any HCB to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzHCB->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
