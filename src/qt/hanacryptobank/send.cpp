...
#include "qt/hanacryptobank/send.h"
#include "qt/hanacryptobank/forms/ui_send.h"
#include "qt/hanacryptobank/addnewcontactdialog.h"
#include "qt/hanacryptobank/qtutils.h"
#include "qt/hanacryptobank/sendchangeaddressdialog.h"
#include "qt/hanacryptobank/optionbutton.h"
#include "qt/hanacryptobank/sendconfirmdialog.h"
#include "qt/hanacryptobank/myaddressrow.h"
#include "qt/hanacryptobank/guitransactionsutils.h"
...
SendWidget::SendWidget(HanaCryptoBankGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (HCB)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("HanaCryptoBank address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 HCB");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_HCB : AskPassphraseDialog::Context::Send_zHCB, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zHCB is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zHCB transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZHCB_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zHCB require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zHCB transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isHCB){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zHCB in your balance to select."));
        }
    }
}
...
void SendWidget::onHCBSelected(bool _isHCB){
    isHCB = _isHCB;
    setCssProperty(coinIcon, _isHCB ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
