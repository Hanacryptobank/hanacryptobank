                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zHCB ought to be enough for anybody." - Bill Gates, 2017
    ui->zHCBpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHCBSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzHCB->setEnabled(false);
        ui->pushButtonMintzHCB->setToolTip(tr("zHCB is currently disabled due to maintenance."));

        ui->pushButtonSpendzHCB->setEnabled(false);
        ui->pushButtonSpendzHCB->setToolTip(tr("zHCB is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHCBpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHCB_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHCB is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHCB...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHCB in ") +
...
void PrivacyDialog::on_pushButtonSpendzHCB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHCB is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zHCB
        sendzHCB();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHCB
    sendzHCB();
}

void PrivacyDialog::on_pushButtonZHcbControl_clicked()
{
    ZHcbControlDialog* zHcbControl = new ZHcbControlDialog(this);
    zHcbControl->setModel(walletModel);
    zHcbControl->exec();
}

void PrivacyDialog::setZHcbControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHcbSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHCB()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid HanaCryptoBank Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zHCBpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHCB is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " HCB </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHCB</b>";
    ...
    // use mints from zHcb selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHcbControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZHcbControlDialog::GetSelectedMints();
    }
    ...
    // Clear zhcb selector in case it was used
    ZHcbControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHcb Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHCB </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHCB "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHCB "));
    ui->labelzHCBAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzHCBSyncStatus->setVisible(fShow);
}
