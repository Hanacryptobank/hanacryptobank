...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZHCBModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpivTracker->HasMintTx(hash))) {
            ...
            sub.type = TransactionRecord::StakeZHCB;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by HanaCryptoBank Address
        ...
        if (fAllFromMeDenom && fAllToMeDenom && ((nFromMe * nToMe) != 0)) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                if (ExtractDestination(wtx.vout[0].scriptPubKey, address)) {
                    // Sent to HanaCryptoBank Address
                    sub.address = CBitcoinAddress(address).ToString();
                } else {
...
bool IsZHCBType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZHCB:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZHCB ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZHCB);
}
