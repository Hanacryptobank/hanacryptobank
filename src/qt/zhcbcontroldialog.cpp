...
#include "ui_zhcbcontroldialog.h"
...
std::list<std::string> ZHcbControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHcbControlDialog::listMints;

ZHcbControlDialog::ZHcbControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHcbControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHcbControlDialog::~ZHcbControlDialog()
{
    delete ui;
}

void ZHcbControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHcbControlDialog::updateList()
{
...
void ZHcbControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHcbControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHcb_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHcbControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHcbControlDialog::GetSelectedMints()
{
...
void ZHcbControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
