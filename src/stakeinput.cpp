...
// The zHCB block index is the first appearance of the accumulator checksum that was used in the spend
...
bool CZPivStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zHCB staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
{
    //The unique identifier for a zHCB is a hash of the serial
...
{
    //Create an output returning the zHCB that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZHCBOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zHCB output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zHCB", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZHCBOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zHCB output", __func__);
...
{
    CzHCBTracker* zpivTracker = pwallet->zpivTracker.get();
    CMintMeta meta;
...
{
    //The unique identifier for a HCB stake is the outpoint
