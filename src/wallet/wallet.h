...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 HCB at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zHCB send
enum ZerocoinSpendStatus {
    ZHCB_SPEND_OKAY = 0,                            // No error
    ZHCB_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHCB_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHCB_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHCB_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHCB_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHCB_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHCB_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHCB_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHCB_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHCB_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHCB_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHCB_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHCB_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHCB_SPENT_USED_ZHCB = 14,                      // Coin has already been spend
    ZHCB_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZHCB_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZHCBOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzHCBTracker> zpivTracker;
    ...
    void setZWallet(CzHCBWallet* zwallet);
    CzHCBWallet* getZWallet();
    ...
    /// Get 10000 HCB output and keys which can be used for the Masternode
    ...
    /** zHCB reset */
    boost::signals2::signal<void()> NotifyzHCBReset;
