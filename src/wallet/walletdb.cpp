...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("hanacryptobank-wallet");
...
bool CWalletDB::WriteZHCBSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZHCBSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZHCBSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZHCBSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZHCBSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZHCBSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZHCBCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZHCBCount(uint32_t& nCount)
{
