...
public:
    ...
    bool WriteZHCBSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZHCBSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZHCBSeed_deprecated(uint256& seed);
    bool EraseZHCBSeed();
    bool EraseZHCBSeed_deprecated();

    bool WriteZHCBCount(const uint32_t& nCount);
    bool ReadZHCBCount(uint32_t& nCount);
