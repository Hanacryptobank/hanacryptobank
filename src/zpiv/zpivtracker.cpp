// Copyright (c) 2018-2020 The HanaCryptoBank developers
...
CzHCBTracker::CzHCBTracker(std::string strWalletFile)
{
...
CzHCBTracker::~CzHCBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHCBTracker::Init()
{
...
bool CzHCBTracker::Archive(CMintMeta& meta)
{
...
bool CzHCBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHCBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHCBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHCBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzHCBTracker::GetSerialHashes()
{
...
CAmount CzHCBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHCBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHCBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHCBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHCBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHCBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHCBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHCBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHCBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHCBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHCBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzHCBWallet* zHCBWallet)
{
    bool iszHCBWalletInitialized = (NULL != zHCBWallet);
    CMintMeta meta;
    ...
    if (! iszHCBWalletInitialized)
        zHCBWallet = new CzHCBWallet(strWalletFile);
    meta.isSeedCorrect = zHCBWallet->CheckSeed(dMint);
    if (! iszHCBWalletInitialized)
        delete zHCBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzHCBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHCBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHCBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHCBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHCBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHCBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzHCBWallet* zHCBWallet = new CzHCBWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zHCBWallet);
        }
        delete zHCBWallet;
