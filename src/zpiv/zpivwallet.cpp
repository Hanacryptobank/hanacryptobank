...
CzHCBWallet::CzHCBWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHCBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHCBSeed_deprecated()) {
                    LogPrintf("%s: Updated zHCB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzHCBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHCBCount(nCountLastUsed);
    else if (!walletdb.ReadZHCBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHCBWallet::Lock()
{
    seedMaster = 0;
}

void CzHCBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHCBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHCB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHCBWallet::LoadMintPoolFromDB()
{
...
void CzHCBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHCBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHCBWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzHCBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHCB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzHCBWallet::SeedToZHCB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHCBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHCBWallet::GenerateDeterministicZHCB(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHCBWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHCBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzHCBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
